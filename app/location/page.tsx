...
import { MapPin, RefreshCw, Circle, ExternalLink } from 'lucide-react'
...
function parseLocation(text: string): LocationData | null {
  ...
  return { latitude: lat, longitude: lng, accuracy: acc ?? '--', provider: prov ?? '--', time: time ?? '--', mapsUrl: url ?? `https://maps.google.com/?q=${lat},${lng}` }
}
  ...
  const fetchLocation = async () => {
    ...
    try {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        const match = (d.history ?? []).find((h: {command:string;result:string}) => h.command === 'get_location')
        if (match?.result) { setRawText(match.result); setLocation(parseLocation(match.result)); break }
      ...
      <main className="flex-1 page-content overflow-y-auto">
        <div className="max-w-2xl mx-auto px-3 md:px-6 py-4 md:py-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg md:text-xl font-bold text-white flex items-center gap-2"><MapPin size={20} className="text-android-green" /> Location</h2>
              <p className="text-android-muted text-xs mt-0.5">Get GPS location from the connected device</p>
            </div>
          ...
          <button onClick={fetchLocation} disabled={!connected || loading}
            className="flex items-center gap-2 px-4 py-2.5 bg-android-green text-android-bg rounded-lg text-sm font-semibold disabled:opacity-40 disabled:cursor-not-allowed mb-4">
            {loading ? <RefreshCw size={14} className="animate-spin" /> : <MapPin size={14} />}
            {loading ? 'Getting location…' : 'Get Location'}
          </button>

          {!connected && <div className="p-8 text-center text-android-muted text-sm bg-android-surface border border-android-border rounded-xl"><MapPin size={32} className="mx-auto mb-3 text-android-border" />Connect a device to get location</div>}
          ...
          {connected && !location && !rawText && !loading && (
            <div className="p-8 text-center text-android-muted text-sm bg-android-surface border border-android-border rounded-xl">
              ...
              <p className="text-xs mt-2 text-android-muted/60">Device must have GPS enabled and location permission granted</p>
            </div>
          )}
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  {[
                    ...
                    <div key={label} className={full ? 'col-span-2' : ''}>
                      ...
                      <p className="text-android-text font-mono text-sm font-semibold">{value}</p>
                    </div>
                  ))}
                </div>
          ...
          {rawText && !location && (
            <div className="bg-android-surface border border-android-border rounded-xl p-4">
              <p className="text-android-yellow text-xs mb-2">⚠️ Could not parse location</p>
