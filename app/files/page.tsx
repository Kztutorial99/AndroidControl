...
interface FileModal {
  path: string; name: string; loading: boolean; error: string
  textContent: string; b64Content: string
  editing: boolean; editBuffer: string; saving: boolean
}
...
function FilesContent() {
  ...
  const navigate = async (targetPath: string) => {
    ...
    try {
      ...
      // Poll until listing arrives for this exact path
      for (let i = 0; i < 15; i++) {
        await sleep(1000)
        const res  = await fetch(`/api/device/files?deviceId=${selectedId}`)
        const data = await res.json()
  ...
  const downloadFile = () => {
    if (!modal) return
    if (modal.b64Content) {
      const bytes = new Uint8Array(atob(modal.b64Content).split('').map(c => c.charCodeAt(0)))
      const url = URL.createObjectURL(new Blob([bytes]))
      const a = document.createElement('a'); a.href = url; a.download = modal.name; a.click()
      URL.revokeObjectURL(url)
    } else if (modal.textContent) {
      const url = URL.createObjectURL(new Blob([modal.textContent], { type: 'text/plain' }))
      const a = document.createElement('a'); a.href = url; a.download = modal.name; a.click()
      URL.revokeObjectURL(url)
    }
  }
          ...
          <div className="flex gap-1.5 mb-2 overflow-x-auto pb-1">
            {QUICK_PATHS.map(({ label, path: p }) => (
              <button key={p} onClick={() => navigate(p)} disabled={!connected}
                className={`shrink-0 px-2.5 py-1.5 border rounded-lg text-xs transition-colors disabled:opacity-40 font-mono whitespace-nowrap ${p === path ? 'bg-android-blue/10 border-android-blue/40 text-android-blue' : 'bg-android-surface border-android-border text-android-muted hover:text-android-blue hover:border-android-blue/50'}`}>
                {label}
              </button>
            ))}
          </div>
          ...
          <div className="bg-android-surface border border-android-border rounded-xl overflow-hidden">
            <div className="px-3 py-2.5 border-b border-android-border flex items-center gap-2">
              ...
              <button onClick={() => navigate(path)} disabled={!connected} className="p-1.5 rounded-lg hover:bg-android-border/50 disabled:opacity-30 transition-colors shrink-0">
                <RefreshCw size={13} className={`text-android-muted ${navLoading ? 'animate-spin' : ''}`} />
              </button>
            </div>
          ...
          <div className="bg-android-surface border border-android-border rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-auto p-4 min-h-0">
              {modal.loading ? (
                ...
                <textarea value={modal.editBuffer}
