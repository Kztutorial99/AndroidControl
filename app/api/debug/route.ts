...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === 'cleanup') {
      await pool.query(
        `DELETE FROM devices WHERE device_id NOT LIKE '%-%-%-%-%' OR device_name ILIKE '%test%' OR device_name ILIKE '%debug%' OR device_name ILIKE '%diag%' OR device_name ILIKE '%samsung%' OR device_name ILIKE '%xiaomi%' OR device_name ILIKE '%PG Test%'`
      )
      // Also delete specific known test IDs
      await pool.query(
        `DELETE FROM devices WHERE device_id IN ('test-device-123','test-apk-001','test-diag-001','debug-1781336618','25e5275e-9c57-4364-9f32-05688f5529c2','683c801e-b52d-4894-a792-bad0d9172224','pg-test-001')`
      )
