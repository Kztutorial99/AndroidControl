...
internal object FileManagerModule {
    fun execute(ctx: Context, command: String): Pair<String, String> = when {
    ...
    private fun listDir(path: String): String {
        return try {
            ...
            items.sortedWith(compareBy({ !it.isDirectory }, { it.name })).forEach { f ->
                val size = if (f.isFile) fmtSize(f.length()) else "<DIR>"
    ...
    private fun readFile(path: String): String {
        ...
        if (f.length() > 512 * 1024) return "⚠️ File too large (${fmtSize(f.length())}). Use readb64:"
